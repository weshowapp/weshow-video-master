...
export default class extends Base {
  ...
  async getbyuserAction() {
    ...
    let list = await this.model('article').where({creator_id: creator_id}).select();
    if (!think.isEmpty(list)) {
  ...
  async getrandomAction() {
    ...
    let list = await this.model('article').getRandomList(count, type, wxconst.QUIZ_LEVEL_AUTO, openid, onlyself);
    return this.success({
      questList: list
    });
  }
