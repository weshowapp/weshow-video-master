...
def main():
    ...
    print (beginIndex)
    #获取txt文件总行数  
    count = len(open("py/citnews_info_url.txt",'rU').readlines())
    print (count)
    n = 0  
    urlfile = open("py/citnews_info_url.txt",'r')
    ...
    while n < count:
        ...
        try:
            ...
            while m <= endIndex:
                ...
                article_content = soup.find('div', class_="newstext")
                ...
                article_author = soup.find('em', class_="source")
                if article_author:
                    #print con + '\n'
                    ...
                    print ('author')
                    print (author)
                    tm = re.match(u'编译/(.*)  来源：(.*)', author, re.M|re.I)
                    if tm:
                        print (tm)
                        author = tm.group(1)
                        source = tm.group(2)
                ...
                #time
                article_time = soup.find('span', class_="time")
                print ('article_time')
                print (article_time)
                if article_time:
